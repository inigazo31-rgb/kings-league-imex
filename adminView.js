...
export function renderAdminView(container) {
  ...
  function update() {
      ...
      tabContentHtml = `
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 2rem;">
          ...
          <div style="background: var(--bg-secondary); border: 1px solid var(--border-medium); border-radius: var(--radius-sm); padding: 1.75rem; clip-path: var(--clip-chamfer);">
            ...
            <div class="form-group">
              <label class="form-label">Dado del Recreo:</label>
    ...
    // -------------------------------------------------------------
      ...
      const isMOpen = store.market.isOpen;
        ...
        <div style="display: flex; flex-direction: column; gap: 1.5rem; max-width: 800px;">
          <div style="background: var(--bg-secondary); border: 1px solid var(--border-medium); border-radius: var(--radius-sm); padding: 1.75rem; clip-path: var(--clip-chamfer);">
            ...
            <p style="font-size: 0.82rem; color: var(--text-secondary); margin-bottom: 1.25rem;">
              Al abrir el mercado, todos los clubes recuperan automáticamente su <strong>Clausulazo</strong> disponible.
            </p>
            ...
            <div style="display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1.5rem;">
              ...
              <button class="btn ${isMOpen ? 'btn-red' : 'btn-lime'}" id="btnToggleMarketMaster">
                ${isMOpen ? '🔒 Cerrar Mercado Ahora' : '🔔 Abrir Mercado Ahora'}
              </button>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
              <div class="form-group">
                <label class="form-label">Fecha y Hora de Próxima Apertura:</label>
                <input type="datetime-local" id="inputMarketNextOpen" class="form-control" value="${store.market.nextOpeningDate ? store.market.nextOpeningDate.substring(0, 16) : ''}" />
              </div>
              <div class="form-group">
                <label class="form-label">Fecha y Hora de Cierre:</label>
                <input type="datetime-local" id="inputMarketClose" class="form-control" value="${store.market.closingDate ? store.market.closingDate.substring(0, 16) : ''}" />
              </div>
            </div>

            <button class="btn btn-outline-lime" id="btnSaveMarketDates" style="margin-top: 0.5rem;">
              Guardar Fechas del Cronómetro
            </button>
          </div>
    ...
    if (activeTab === "results") {
      ...
      container.querySelector("#btnToggleMarketMaster")?.addEventListener("click", () => {
        const newState = !store.market.isOpen;
        store.setMarketStatus(newState);
        toast.show("Mercado", `El mercado está ahora ${newState ? 'ABIERTO' : 'CERRADO'}`, newState ? "lime" : "red");
        update();
      });

      container.querySelector("#btnSaveMarketDates")?.addEventListener("click", () => {
        const nextO = container.querySelector("#inputMarketNextOpen").value;
        const closeD = container.querySelector("#inputMarketClose").value;
        store.setMarketStatus(store.market.isOpen, nextO, closeD);
        toast.show("Cronómetro", "Fechas oficiales guardadas.", "lime");
        update();
      });
  ...
  function openEditPlayerModal(playerId) {
    ...
    document.getElementById("btnSavePlayerData")?.addEventListener("click", () => {
      ...
      if (isInsignia) {
        store.setInsigniaPlayer(teamId, player.id);
      }
