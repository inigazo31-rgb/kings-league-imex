...
- **Dados del Recreo**: Modalidades 1vs1 con portero, 2vs2 sin porteros, 3vs3 y 4vs4 para los minutos finales del primer tiempo.
...
2. **Desde el archivo de datos base**: Edita [`js/data/defaultData.js`](file:///C:/Users/ifg31/.gemini/antigravity/scratch/kings-league-imex/js/data/defaultData.js) para alterar los nombres, escudos, dorsales, presidentes y valores por defecto del torneo.
...
## 🔌 5. Conexión Futura a Backend (Supabase / Firebase)

La plataforma utiliza el patrón **Store Centralizado** en [`js/state/store.js`](file:///C:/Users/ifg31/.gemini/antigravity/scratch/kings-league-imex/js/state/store.js). Para migrar a una base de datos en tiempo real:
- Sustituye los métodos `saveState()` y `loadState()` por llamadas a las tablas de Supabase (`supabase.from('teams').select(...)`) o Firestore.
- Toda la capa visual (`views/` y `components/`) se mantendrá intacta sin requerir reescrituras.
