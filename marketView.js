...
export function renderMarketView(container) {
  ...
  const isMarketOpen = store.market.isOpen;
  ...
  function update() {
    ...
    const statusText = isMarketOpen ? "MERCADO OFICIALMENTE ABIERTO" : "MERCADO CERRADO";
    const targetDate = isMarketOpen ? store.market.closingDate : store.market.nextOpeningDate;
    ...
    if (activeTab === "clausulazos") {
      // Catálogo de jugadores para clausulazo
      const availablePlayers = store.players.filter((p) => {
        if (userTeam && p.teamId === userTeam.id) return false;
        return true;
      });
      ...
      tabContentHtml = `
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.25rem;">
          ${availablePlayers.map((p) => {
            ...
            const canClausulazo = isMarketOpen && (!userTeam || (!userTeam.clausulazoUsed && canAfford && !p.isInsignia));
            ...
            return `
              <div style="background: var(--bg-secondary); border: 1px solid ${p.isInsignia ? 'rgba(255,215,0,0.4)' : 'var(--border-subtle)'}; border-radius: var(--radius-sm); padding: 1.25rem; clip-path: var(--clip-chamfer); display: flex; flex-direction: column; gap: 0.75rem;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.3); padding: 0.5rem 0.75rem; border-radius: var(--radius-xs); border: 1px solid var(--border-subtle);">
                  <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 700;">VALOR CLÁUSULA:</span>
      ...
      tabContentHtml = `
        <div style="background: var(--bg-secondary); border: 1px solid var(--border-medium); border-radius: var(--radius-sm); padding: 2rem; max-width: 720px; margin: 0 auto; clip-path: var(--clip-chamfer);">
          ...
          <div class="form-group">
            ...
            <input type="text" id="inputTransferNotes" class="form-control" placeholder="Ej: Pago de contado en el próximo recreo..." />
          </div>

          <button class="btn btn-lime btn-lg" id="btnSendTransferProposal" style="width: 100%; margin-top: 1rem;">
