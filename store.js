...
class Store {
  ...
  loadState() {
    try {
      ...
      if (saved) {
        ...
        this.teams = parsed.teams || DEFAULT_TEAMS;
        this.players = parsed.players || DEFAULT_PLAYERS;
        this.matches = parsed.matches || DEFAULT_MATCHES;
        this.market = parsed.market || DEFAULT_MARKET_STATE;
        this.finances = parsed.finances || DEFAULT_FINANCES;
        this.rulesCards = parsed.rulesCards || DEFAULT_RULES_CARDS;
        this.diceRules = parsed.diceRules || DEFAULT_DICE_RULES;
        this.announcements = parsed.announcements || DEFAULT_ANNOUNCEMENTS;
        this.auctionPlayers = parsed.auctionPlayers || DEFAULT_AUCTION_PLAYERS;
        this.users = parsed.users || DEFAULT_USERS;
        this.notifications = parsed.notifications || this.generateDefaultNotifications();
    ...
    // Datos por defecto si no hay storage
  ...
  generateDefaultNotifications() {
    return [
      ...
      { id: "n-4", title: "Próxima Jornada IMEX", message: "Jornada 02 programada para este jueves en el recreo central.", time: "Hace 5 horas", read: true, type: "match" }
    ];
  }
  ...
  switchRole(roleType, targetId = null) {
    if (roleType === "ADMIN") {
      this.currentUser = {
        id: "u-admin",
        name: "Prof. Alejandro Valdés",
        role: "ADMIN",
        teamId: null,
        playerId: null,
        avatar: "👑",
      ...
      this.currentUser = {
        id: `u-presi-${team.id}`,
        name: `${team.president} (Pres. ${team.name})`,
        role: "PRESIDENT",
        teamId: team.id,
        playerId: null,
        avatar: team.logoText || "🦅",
      };
  ...
  recordMatchResult(matchData) {
    ...
    if (homeTeam && awayTeam && !wasFinished) {
      ...
      this.addNotification({
        ...
        message: `${homeTeam.name} ${matchData.homeScore} - ${matchData.awayScore} ${awayTeam.name}. Premios acreditados (+${homeGain}M / +${awayGain}M).`,
        type: "match"
      });
    }
  ...
  setMarketStatus(isOpen, nextDate = null, closeDate = null) {
    this.market.isOpen = Boolean(isOpen);
    if (nextDate) this.market.nextOpeningDate = nextDate;
    if (closeDate) this.market.closingDate = closeDate;

    // Si se abre el mercado, se resetean los clausulazos para una nueva ventana
    if (this.market.isOpen) {
      this.teams.forEach((t) => (t.clausulazoUsed = false));
      this.addNotification({
        title: "🔔 ¡MERCADO DE FICHAJES ABIERTO!",
        message: "El Organizador ha habilitado la ventana de fichajes. Todos los clubes recuperan su Clausulazo.",
        type: "market"
      });
    } else {
      this.addNotification({
        title: "🔒 Mercado Cerrado",
        message: "La ventana de fichajes ha concluido oficialmente.",
        type: "market"
      });
    }
  ...
  adjustTeamFinance(teamId, amount, concept, isIncome = true) {
    ...
    this.finances.unshift({
      ...
      concept: concept || (isIncome ? "Ajuste manual de fondos" : "Sanción económica de torneo"),
      amount: val,
      date: new Date().toISOString().split("T")[0],
    });
  ...
  updateTeam(teamData) {
    ...
    if (idx === -1) {
      const newT = {
        ...
        shortName: teamData.shortName || "NEQ",
        president: teamData.president || "Presidente",
        group: teamData.group || "A",
        color: teamData.color || "#00FF66",
        secondaryColor: teamData.secondaryColor || "#111111",
  ...
  setGenerationLogo(logoUrlOrData) {
    this.league.generationLogo = logoUrlOrData;
    this.saveState();
  }
  ...
  addAnnouncement(annData) {
    const newAnn = {
      ...
      author: annData.author || "Comité IMEX",
    };
